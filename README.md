...
本次版本完成 Windows 桌宠 MVP 的阶段性收口，主要变更如下：

- 改版 WPF 桌宠主界面，接入 7 套角色状态图、文本输入、语音入口和统一状态展示
- 跑通文本、录音、STT、LLM、TTS 与本地播放链路，并复用现有 Provider 与 Pipeline
- 新增本地会话历史、最近 6 轮连续文本对话和新建会话能力
- 完善错误气泡与恢复操作，补充鉴权错误分类、重试、打开设置和重试播放
- 新增 Windows 一键启动和环境自检，启动前检查项目结构、配置、虚拟环境与运行目录
- 补充自动化测试与使用文档；本地 Key、运行配置、日志、会话、录音和生成音频继续由 Git 忽略

本次版本不包含安装包、自动安装依赖、实时双工语音、流式 RVC、Android UI 或云端会话同步。
