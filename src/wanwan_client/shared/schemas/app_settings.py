...
class ServiceSettingsGroup:
    ...
    enabled: bool = True
    default_provider_id: str = "(填写默认 Provider ID)"
    ...
    def from_dict(cls, data: dict[str, Any] | None) -> "ServiceSettingsGroup":
        ...
        return cls(
            ...
            default_provider_id=str(source.get("default_provider_id", "(填写默认 Provider ID)")),
            providers=[
...
class AppSettings:
    ...
    schema_version: str = "settings.v0.2"
    ...
    def from_dict(cls, data: dict[str, Any] | None) -> "AppSettings":
        source = data or {}
        settings = cls(
            schema_version=str(source.get("schema_version", "settings.v0.2")),
